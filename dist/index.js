...
;// CONCATENATED MODULE: ./src/sleep.mjs
function sleep(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms));
}

;// CONCATENATED MODULE: ./src/_phrase_utils.mjs
...
const uploadLocaleFile = async () => {
  ...
  const fileStream = external_fs_.createReadStream('./src/locales/en.json');
  ...
  // https://developers.phrase.com/api/#post-/projects/-project_id-/uploads
  const uploadResponse = await node_ponyfill(`${BASE_API_URL}/uploads`, {
    method: 'POST',
    headers: {
...
const checkUploadStatus = async (uploadId) => {
  ...
  let uploadStatusResponse = await node_ponyfill(
  ...
  for (let i = 0; i < MAX_RETRIES; i += 1) {
    ...
    uploadStatusResponse = await node_ponyfill(`${BASE_API_URL}/uploads/${uploadId}`, {
      headers: {
        Authorization: `token ${TOKEN}`,
      },
    });
...
const excludeKeysInLocale = async (
  locale,
  uploadId,
  nUnmentionedKeys,
) => {
  ...
  const excludeResponse = await node_ponyfill(`${BASE_API_URL}/keys/exclude`, {
    method: 'PATCH',
    headers: {
      Authorization: `token ${TOKEN}`,
...
const includeKeysInLocale = async (locale) => {
  ...
  const includeResponse = await node_ponyfill(`${BASE_API_URL}/keys/include`, {
    method: 'PATCH',
    headers: {
      Authorization: `token ${TOKEN}`,
      'Content-Type': 'application/json',
...
const run = async () => {
  try {
    const reset = core_default().getInput('reset');
    if (reset) {
...
const phraseResetExclusions = async () => {
  ...
  core_default().info('Successfully re-included all keys.');
};
